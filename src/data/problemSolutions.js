...
`
  ...
  // ── SQL CHALLENGES (36-45) ────────────────────────────────────────────────
  "prob-sql-high-earners": {
    tags: ["SQL", "Aggregation", "Filtering"],
    inputFormat: "Table `employees (id, name, department, salary)`",
    outputFormat: "Table with columns `name, salary` where salary > 75000 ordered by salary DESC.",
    constraints: [
      "Filter: salary > 75000",
      "Order: salary DESC",
      "Syntax: Standard ANSI SQL"
    ],
    examples: [
      {
        input: "SELECT name, salary FROM employees WHERE salary > 75000 ORDER BY salary DESC;",
        output: "Rows matching salary > 75000 ordered highest first.",
        explanation: "Filters the employees relation by the threshold and orders the result set descending by compensation."
      }
    ],
    editorial: {
      intuition: "Use standard SQL projection (`SELECT`), row filtering (`WHERE`), and sorting (`ORDER BY`).",
      algorithm: [
        "1. Specify projection: `SELECT name, salary`.",
        "2. Specify table source: `FROM employees`.",
        "3. Apply predicate: `WHERE salary > 75000`.",
        "4. Sort result: `ORDER BY salary DESC;`."
      ],
      timeComplexity: "O(N log N) due to sorting",
      spaceComplexity: "O(N) for result set",
      complexityExplanation: "Database index on salary can optimize the WHERE and ORDER BY to O(K) where K is number of matches.",
...
`
  ...
  "prob-sql-highest-paid-instructor": {
    tags: ["SQL", "Aggregation", "ORDER BY", "LIMIT"],
    inputFormat: "Table `instructors (id, name, department, salary)`",
    outputFormat: "Table with column `name` of the instructor with highest salary.",
    constraints: [
      "Single row result",
      "Order by salary DESC",
      "Limit 1"
    ...
    examples: [
      {
        input: "SELECT name FROM instructors ORDER BY salary DESC LIMIT 1;",
        output: "Highest paid instructor name",
        explanation: "Sorts records in descending order of compensation and limits output to the top 1."
      }
    ],
    editorial: {
      intuition: "Sort records by salary in descending order and limit the result set to 1 row.",
      algorithm: [
        "1. Write projection: `SELECT name`.",
        "2. Identify relation: `FROM instructors`.",
        "3. Order descending: `ORDER BY salary DESC`.",
        "4. Restrict to 1 record: `LIMIT 1;`."
      ],
      timeComplexity: "O(N log N) without index, O(1) with index",
      spaceComplexity: "O(1)",
      complexityExplanation: "Constant memory overhead with index-backed scan.",
      solutionCode: `SELECT name FROM instructors ORDER BY salary DESC LIMIT 1;\n`
    }
  }
};
